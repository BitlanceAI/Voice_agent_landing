  ...
  return (
    <section id="faq" className="py-20 px-4 bg-white">
      <div className="max-w-3xl mx-auto">
        ...
        <p className="text-center text-slate-600 -mt-6 mb-10">
          Quick answers about setup, integrations, handoff to humans, and security.
        </p>
        <div className="space-y-4">
          {faqs.map((faq, index) => (
            <div key={index} className="glass rounded-2xl overflow-hidden border border-slate-200">
              <button 
                onClick={() => setOpenIndex(openIndex === index ? null : index)}
                aria-expanded={openIndex === index}
                aria-controls={`faq-panel-${index}`}
                className="w-full text-left p-6 flex justify-between items-center hover:bg-slate-50 transition-colors focus:outline-none focus:ring-2 focus:ring-primary/30"
              >
                <h4 className="font-black text-slate-900 pr-4">{faq.question}</h4>
                <motion.span
              ...
              <AnimatePresence>
                {openIndex === index && (
                  ...
                  >
                    <div
                      id={`faq-panel-${index}`}
                      className="p-6 pt-0 text-slate-600 leading-relaxed border-t border-slate-200"
                    >
                      {faq.answer}
                    </div>
                  </motion.div>
        ...
        <div className="mt-12 text-center">
          <div className="glass rounded-3xl p-8 border border-slate-200">
            <p className="text-lg font-black text-slate-900">Want a tailored recommendation?</p>
